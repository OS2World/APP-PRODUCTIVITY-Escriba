    ...
    u_short minsamplevalue, u_short maxsamplevalue, RGBvalue ** pMap )
{
   ...
   if ( bitspersample <= 8 &&
      ...
      /* Use photometric mapping table to construct unpacking tables for
       * samples <= 8 bits. */
...
static int gt ( TIFF * tif, int w, int h, u_long * raster )
{
   ...
   switch ( photometric ) {
      ...
      case PHOTOMETRIC_PALETTE:
         ...
         /* Convert 16-bit colormap to 8-bit (unless it looks like an
          * old-style 8-bit colormap). */
   ...
   for ( row = 0; row < h; row += th ) {
      ...
      for ( col = 0; col < w; col += tw ) {
         if ( TIFFReadTile ( tif, buf, col, row, 0, 0 ) < 0 &&
            ...
            break;
         if ( col + tw > w ) {
            /* Tile is clipped horizontally.  Calculate visible portion
             * and skewing factors. */
            u_long npix = w - col;
 ...
 * number.
 */
static int makebwmap ( TIFF * tif, RGBvalue * Map )
{
   register int i;
 ...
 * number.
 */
static int makecmap ( TIFF * tif, u_short * rmap, u_short * gmap, u_short * bmap )
{
   register int i;
   int nsamples = 8 / bitspersample;
   register u_long *p;
