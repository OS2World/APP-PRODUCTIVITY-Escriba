 ...
 */
static void TIFFInitOrder ( register TIFF * tif, int magic, int bigendian )
{
 ...
 )
{
   ...
   /* Now check version (if needed, it's been byte-swapped). Note that
    * this isn't actually a version number, it's a magic number that
    * doesn't change (stupid). */
   ...
   switch ( mode[0] ) {
      case 'r':
         ...
         break;
      case 'a':
         /* Don't append to file that has information byte swapped -- we
          * will write data that is in the opposite order. */
         if ( tif->tif_flags & TIFF_SWAB ) {
            TIFFError ( name,
               "Cannot append to file that has opposite byte ordering" );
            goto bad;
         }
         /* New directories are automatically append to the end of the
          * directory chain when they are written out (see
          * TIFFWriteDirectory). */
         if ( !TIFFDefaultDirectory ( tif ) )
            goto bad;
         return ( tif );
