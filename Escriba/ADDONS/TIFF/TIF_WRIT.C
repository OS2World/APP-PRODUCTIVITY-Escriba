...
int TIFFWriteScanline ( TIFF * tif, tdata_t buf, uint32 row, tsample_t sample )
{
   ...
   if ( strip != tif->tif_curstrip ) {
      ...
      /* Watch out for a growing image.  The value of strips/image will
       * initially be 1 (since it can't be deduced until the imagelength
       * is known). */
   ...
   if ( strip >= td->td_nstrips && !TIFFGrowStrips ( tif, 1, module ) )
      return ( -1 );
   /* Ensure the write is either sequential or at the beginning of a strip
    * (or that we can randomly access the data -- i.e. no encoding). */
   if ( row != tif->tif_row ) {
      if ( tif->tif_seek ) {
         if ( row < tif->tif_row ) {
            /* Moving backwards within the same strip: backup to the start
             * and then decode forward (below). */
...
TIFFWriteEncodedStrip ( TIFF * tif, tstrip_t strip, tdata_t data, tsize_t cc )
{
   ...
   /* Handle delayed allocation of data buffer.  This permits it to be
    * sized according to the directory info. */
   if ( ( tif->tif_flags & TIFF_BUFFERSETUP ) == 0 ) {
      if ( !TIFFBufferSetup ( tif, module ) )
         return ( ( tsize_t ) - 1 );
      tif->tif_flags |= TIFF_BUFFERSETUP;
...
TIFFWriteEncodedTile ( TIFF * tif, ttile_t tile, tdata_t data, tsize_t cc )
{
   ...
   if ( tif->tif_preencode && !( *tif->tif_preencode ) ( tif ) )
      return ( ( tsize_t ) - 1 );
   /* Clamp write amount to the tile size.  This is mostly done so that
    * callers can pass in some large number (e.g. -1) and have the tile
    * size used instead. */
...
static int TIFFWriteCheck ( TIFF * tif, int tiles, const char *module )
{
   ...
   /* On the first write verify all the required information has been
    * setup and initialize any data structures that had to wait until
    * directory information was set. Note that a lot of our work is
    * assumed to remain valid because we disallow any of the important
    * parameters from changing after we start writing (i.e. once
    * TIFF_BEENWRITING is set, TIFFSetField will only allow the image's
 ...
 */
static int TIFFBufferSetup ( TIFF * tif, const char *module )
{
   tsize_t size;
 ...
 */
void TIFFSetWriteOffset ( TIFF * tif, off_t off )
{
   tif->tif_curoff = off;
}
