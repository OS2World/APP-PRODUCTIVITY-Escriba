...
static int TIFFSeek ( TIFF * tif, uint32 row, tsample_t sample )
{
   ...
   if ( strip != tif->tif_curstrip ) {  /* different strip, refill */
      ...
      /* Moving backwards within the same strip: backup to the start and
       * then decode forward (below).
       * 
       * NB: If you're planning on lots of random access within a strip, it's
       * better to just read and decode the entire strip, and then access
       * the decoded data in a random fashion. */
...
TIFFReadEncodedStrip ( TIFF * tif, tstrip_t strip, tdata_t buf, tsize_t size )
{
   ...
   /* Calculate the strip size according to the number of rows in the
    * strip (check for truncated last strip). */
...
static int TIFFFillStrip ( TIFF * tif, tstrip_t strip )
{
   ...
   if ( isMapped ( tif ) &&
      ...
      /* The image is mapped into memory and we either don't need to flip
       * bits or the compression routine is going to handle this operation
       * itself.  In this case, avoid copying the raw data and instead
       * just reference the data from the memory mapped file image.  This
       * assumes that the decompression routines do not modify the
       * contents of the raw data buffer (if they try to, the application
       * will get a fault since the file is mapped read-only). */
      if ( ( tif->tif_flags & TIFF_MYBUFFER ) && tif->tif_rawdata )
         _TIFFfree ( tif->tif_rawdata );
      tif->tif_flags &= ~TIFF_MYBUFFER;
      if ( td->td_stripoffset[strip] + bytecount > tif->tif_size ) {
         /* This error message might seem strange, but it's what would
          * happen if a read were done instead. */
      ...
      /* Expand raw data buffer, if needed, to hold data strip coming from
       * file (perhaps should set upper bound on the size of a buffer
       * we'll use?). */
...
static int TIFFFillTile ( TIFF * tif, ttile_t tile )
{
   ...
   if ( isMapped ( tif ) &&
      ...
      /* Expand raw data buffer, if needed, to hold data tile coming from
       * file (perhaps should set upper bound on the size of a buffer
       * we'll use?). */
      if ( bytecount > tif->tif_rawdatasize ) {
         tif->tif_curtile = -1; /* unknown state */
...
void TIFFNoPostDecode ( TIFF * tif, tidata_t buf, tsize_t cc )
{
}

void TIFFSwab16BitData ( TIFF * tif, tidata_t buf, tsize_t cc )
{
   assert ( ( cc & 1 ) == 0 );
   TIFFSwabArrayOfShort ( ( uint16 * ) buf, cc / 2 );
}

void TIFFSwab32BitData ( TIFF * tif, tidata_t buf, tsize_t cc )
{
   assert ( ( cc & 3 ) == 0 );
