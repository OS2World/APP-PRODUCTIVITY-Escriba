...
static void setString ( char **cpp, char *cp )
{
   if ( *cpp )
      _TIFFfree ( *cpp ), *cpp = 0;
   if ( cp ) {
...
static void setShortArray ( uint16 ** wpp, uint16 * wp, long n )
{
   if ( *wpp )
...
#if 0                                                           /* RWP: Removed 7 May 1996 because unused. */
                                                                /* RWP: Removed 7 May 1996 because unused. */
static void setLongArray ( uint32 ** wpp, uint32 * wp, long n ) /* RWP: Removed 7 May 1996 because unused. */
{                                                               /* RWP: Removed 7 May 1996 because unused. */
   if ( *wpp )                                                  /* RWP: Removed 7 May 1996 because unused. */
      _TIFFfree ( ( char * ) *wpp ), *wpp = 0;                  /* RWP: Removed 7 May 1996 because unused. */
   n *= sizeof ( uint32 );                                      /* RWP: Removed 7 May 1996 because unused. */
   if ( wp && ( *wpp = ( uint32 * ) _TIFFmalloc ( n ) ) )       /* RWP: Removed 7 May 1996 because unused. */
      memcpy ( *wpp, wp, n );                                   /* RWP: Removed 7 May 1996 because unused. */
}                                                               /* RWP: Removed 7 May 1996 because unused. */
                                                                /* RWP: Removed 7 May 1996 because unused. */
#endif                                                          /* RWP: Removed 7 May 1996 because unused. */

static void setFloatArray ( float **wpp, float *wp, long n )
{
   if ( *wpp )
      _TIFFfree ( ( char * ) *wpp ), *wpp = 0;
   n *= sizeof ( float );
 ...
 */
static void setJPEGQTable ( u_char *** wpp, u_char ** wp, int nc )
{
 ...
 */
static void setJPEGCTable ( u_char *** cpp, u_char ** cp, int nc )
{
   u_char *tab;
   int i, j, nw;
   ...
   if ( *cpp )
      _TIFFfree ( *cpp ), *cpp = 0;
   /* Calculate the size of the table by counting the number of codes
    * specified in the bits array. */
   nw = 0;
   for ( i = 0; i < nc; i++ ) {
...
static int TIFFSetField1 ( TIFF * tif, ttag_t tag, va_list ap )
{
   ...
   switch ( tag ) {
      ...
      case TIFFTAG_BITSPERSAMPLE:
          ...
          * samples, set it up here.  Note that since tags are required to
          * be ordered, compression code can override this behaviour in
          * the setup method if it wants to roll the post decoding work in
...
static int OkToChangeTag ( TIFF * tif, ttag_t tag )
{
   if ( tag != TIFFTAG_IMAGELENGTH &&
       ...
       * don't/shouldn't affect the compression and/or format of the data. */
...
static void TIFFGetField1 ( TIFFDirectory * td, ttag_t tag, va_list ap )
{
   switch ( tag ) {
 case TIFFTAG_SUBFILETYPE:
 ...
 */
void _TIFFgetfield ( TIFFDirectory * td, ttag_t tag,... )
{
   va_list ap;
...
int TIFFDefaultDirectory ( TIFF * tif )
{
   ...
   /* NB: The directory is marked dirty as a result of setting up the
    * default compression scheme.  However, this really isn't correct --
    * we want TIFF_DIRTYDIRECT to be set only if the user does something.
    * We could just do the setup by hand, but it seems better to use the
    * normal mechanism (i.e. TIFFSetField). */
   tif->tif_flags &= ~TIFF_DIRTYDIRECT;
   return ( 1 );
}
...
int TIFFSetDirectory ( TIFF * tif, tdir_t dirn )
{
    ...
    * TIFFReadDirectory will increment tif_curdir after successfully
