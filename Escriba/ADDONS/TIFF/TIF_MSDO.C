...
#include <io.h>                 /* for open, close, etc. function
                                 * prototypes */
#endif
...
{
   return ( _read ( ( int ) fd, buf, size ) );
}

static tsize_t
 _tiffWriteProc ( thandle_t fd, tdata_t buf, tsize_t size )
{
   return ( _write ( ( int ) fd, buf, size ) );
}

static toff_t
 _tiffSeekProc ( thandle_t fd, toff_t off, int whence )
{
   return ( _lseek ( ( int ) fd, ( off_t ) off, whence ) );
}
