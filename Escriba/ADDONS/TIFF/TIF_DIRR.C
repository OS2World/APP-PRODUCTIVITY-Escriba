...
int TIFFReadDirectory ( TIFF * tif )
{
   ...
   /* Setup default value and then make a pass over the fields to check
    * type and tag information, and to extract info required to size data
    * structures.  A second pass is made afterwards to read in everthing
    * not taken in the first pass. */
   td = &tif->tif_dir;
   /* free any old stuff and reinit */
   TIFFFreeDirectory ( tif );
   TIFFDefaultDirectory ( tif );
   tif->tif_postdecode = TIFFNoPostDecode;
   /* Electronic Arts writes gray-scale TIFF files without a
    * PlanarConfiguration directory entry. Thus we setup a default value
    * here, even though the TIFF spec says there is no default value. */
   ...
   for ( fip = tiffFieldInfo, dp = dir, n = dircount; n > 0; n--, dp++ ) {
      ...
      /* Silicon Beach (at least) writes unordered directory tags
       * (violating the spec).  Handle it here, but be obnoxious (maybe
       * they'll fix it?). */
   ...
   for ( dp = dir, n = dircount; n > 0; n--, dp++ ) {
      ...
      switch ( dp->tdir_tag ) {
         ...
         case TIFFTAG_BITSPERSAMPLE:
             ...
             * this, we accept the tag if one value is supplied.
             * 
             * The MinSampleValue, MaxSampleValue and BitsPerSample tags are
             * supposed to be written as one value/sample, but some
             * vendors incorrectly write one value only -- so we accept
             * that as well (yech). */
            if ( dp->tdir_count == 1 ) {
         ...
         case TIFFTAG_TRANSFERFUNCTION:
            ...
            if ( cp != NULL ) {
               if ( TIFFFetchData ( tif, dp, cp ) ) {
                  /* This deals with there being only one array to apply
                   * to all samples. */
...
/* END REV 4.0 COMPATIBILITY */
   ...
   if ( !TIFFFieldSet ( tif, FIELD_STRIPBYTECOUNTS ) ) {
      /* Some manufacturers violate the spec by not giving the size of the
       * strips.  In this case, assume there is one uncompressed strip of
       * data. */
      if ( td->td_nstrips > 1 ) {
...
#define BYTECOUNTLOOKSBAD \
    ...
    (td->td_compression == COMPRESSION_NONE && \
     td->td_stripbytecount[0] > TIFFGetFileSize(tif) - td->td_stripoffset[0]))
      ...
      /* Plexus (and others) sometimes give a value of zero for a tag when
       * they don't know what the correct value is!  Try and handle the
       * simple case of estimating the size of a one strip image. */
      TIFFWarning ( tif->tif_name,
...
#ifdef STRIPCHOP_SUPPORT
   /* Some manufacturers make life difficult by writing large amounts of
    * uncompressed data as a single strip. This is contrary to the
    * recommendations of the spec. The following makes an attempt at
    * breaking such images into strips closer to the recommended 8k bytes.
    * A side effect, however, is that the RowsPerStrip tag value may be
    * changed. */
...
static void EstimateStripByteCounts ( TIFF * tif, TIFFDirEntry * dir, uint16 dircount )
{
   ...
   if ( td->td_compression != COMPRESSION_NONE ) {
      ...
      /* This gross hack handles the case were the offset to the strip is
       * past the place where we think the strip should begin.  Since a
       * strip of data must be contiguous, it's safe to assume that we've
       * overestimated the amount of data in the strip and trim this
       * number back accordingly. */
   ...
   if ( dp->tdir_count > 1 ) {  /* array of values */
      ...
      switch ( dp->tdir_type ) {
         ...
         case TIFF_FLOAT:
            ...
            break;
         case TIFF_ASCII:
            /* Some vendors write strings w/o the trailing NULL byte, so
             * always append one just in case. */
      ...
      switch ( dp->tdir_type ) {
         ...
         case TIFF_SHORT:
         case TIFF_SSHORT:
            /* If the tag is also acceptable as a LONG or SLONG then
             * TIFFSetField will expect an uint32 parameter passed to it
             * (through varargs).  Thus, for machines where sizeof (int)
             * != sizeof (uint32) we must do a careful check here.  It's
             * hard to say if this is worth optimizing.
             * 
             * NB: We use TIFFFieldWithTag here knowing that it returns us
             * the first entry in the table for the tag and that that
             * entry is for the widest potential data type the tag may
             * have. */
...
static int TIFFFetchJPEGQTables ( TIFF * tif, TIFFDirEntry * dir )
{
   ...
   /* We don't share per-component q matrices because (besides
    * complicating this logic even more), it would make it very painful if
    * the user does a ``set''. */
...
static int TIFFFetchJPEGCTables ( TIFF * tif, TIFFDirEntry * dir, u_char *** ptab )
{
   ...
   if ( dir->tdir_count > 1 ) {
      ...
      off[0] = dir->tdir_offset;
   /* We don't share per-component tables because (besides complicating
    * this logic even more), it would make it very painful if the user
    * does a ``set''.  Note also that we don't try to optimize storage of
    * the tables -- we just allocate enough space to hold the largest
    * possible.  All this stuff is so complicated 'cuz the tag is defined
    * to be compatible with the JPEG table format, rather than something
    * that fits well into the structure of TIFF -- argh! */
   *ptab = ( u_char ** ) CheckMalloc ( tif, dir->tdir_count *
   ...
   for ( i = 0; i < dir->tdir_count; i++ ) {
      ...
      tdir.tdir_offset = off[i];
      tdir.tdir_count = 16;
      /* We must fetch the array that holds the count of codes for each
       * bit length first and the count up the number of codes that are in
       * the variable length table.  This information is implicit in the
       * JPEG format 'cuz it's preceded by a length field. */
...
static void ChopUpSingleUncompressedStrip ( TIFF * tif )
{
   ...
   /* Make the rows hold at least one scanline, but fill 8k if possible. */
   if ( rowbytes > 8192 ) {
      stripbytes = rowbytes;
      rowsperstrip = 1;
   } else {
   ...
   if ( newcounts == NULL || newoffsets == NULL ) {
      /* Unable to allocate new strip information, give up and use the
       * original one strip information. */
      if ( newcounts != NULL )
         _TIFFfree ( newcounts );
      if ( newoffsets != NULL )
