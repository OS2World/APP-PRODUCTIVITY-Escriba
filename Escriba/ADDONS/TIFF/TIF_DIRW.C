...
int TIFFWriteDirectory ( TIFF * tif )
{
   ...
   if ( tif->tif_cleanup )
      ( *tif->tif_cleanup ) ( tif );
   /* Flush any data that might have been written by the compression
    * close+cleanup routines. */
   ...
   /* Directory hasn't been placed yet, put it at the end of the file and
    * link it into the existing directory structure. */
   ...
   /* Write out ExtraSamples tag only if extra samples are present in the
    * data. */
   ...
   for ( fip = tiffFieldInfo; fip->field_tag; fip++ ) {
      ...
      switch ( fip->field_bit ) {
         case FIELD_STRIPOFFSETS:
            /* We use one field bit for both strip and tile offsets, and
             * so must be careful in selecting the appropriate field
             * descriptor (so that tags are written in sorted order). */
            tag = isTiled ( tif ) ?
         ...
         case FIELD_STRIPBYTECOUNTS:
            /* We use one field bit for both strip and tile byte counts,
             * and so must be careful in selecting the appropriate field
             * descriptor (so that tags are written in sorted order). */
            tag = isTiled ( tif ) ?
 ...
 */
static int TIFFWriteRational ( TIFF * tif,
 ...
 */
static int TIFFWriteShortTable ( TIFF * tif,
 ...
 */
static int TIFFWriteString ( TIFF * tif, ttag_t tag, TIFFDirEntry * dir, char *cp )
{
   dir->tdir_tag = tag;
 ...
 */
static int TIFFWriteShortArray ( TIFF * tif,
 ...
 */
static int TIFFWriteLongArray ( TIFF * tif,
 ...
 */
static int TIFFWriteRationalArray ( TIFF * tif,
...
static int TIFFWriteFloatArray ( TIFF * tif,
    TIFFDataType type, ttag_t tag, TIFFDirEntry * dir, uint32 n, float *v )
{
   dir->tdir_tag = tag;
   dir->tdir_type = ( short ) type;
 ...
 */
static int TIFFWriteJPEGQTables ( TIFF * tif, TIFFDirEntry * dir )
{
 ...
 * tables and write the associated indirect values.
 */
static int TIFFWriteJPEGCTables ( TIFF * tif, ttag_t tag, TIFFDirEntry * dir, u_char ** tab )
{
...
static int TIFFWriteTransferFunction ( TIFF * tif, TIFFDirEntry * dir )
{
   ...
   /* Check if the table can be written as a single column, or if it must
    * be written as 3 columns.  Note that we write a 3-column tag if there
    * are 2 samples/pixel and a single column of data won't suffice--hmm. */
   switch ( td->td_samplesperpixel - td->td_extrasamples ) {
      default:
         if ( memcmp ( tf[0], tf[2], n ) ) {
            ncols = 3;
            break;
         }
      case 2:
         if ( memcmp ( tf[0], tf[1], n ) ) {
            ncols = 3;
            break;
         }
      case 1:
